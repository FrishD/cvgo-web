...
vec4 plusLighter(vec4 source, vec4 destination) {
	// Increase the strength of the source color
	source.rgb *= uAlpha;
	// Make the destination (original layer) compcoely transparent
	destination.a = 1.0;
	//destination.a  = destination.rgb*3.0;
	vec4 finalColor = source + destination * (1.0 - source.a);

	// Just show the white part of the text
...
void main() {
	...
	// Calculate normalized distance from mouse position
	vec2 diff = uv - mousePos;
	float dist = length(diff);
	
	// Create a circular blur area with smooth edges
	float blurSize = uBlurSize * 0.5; // Adjusted for circular shape
	float innerRadius = blurSize * 0.2;
	float outerRadius = blurSize;
	
	float blurFactor = 1.0 - smoothstep(innerRadius, outerRadius, dist);
	
	// Get the original texture color
	vec4 originalColor = texture2D(uOriginalTexture, uv);
	
	// If outside blur area, just use original texture
	if (blurFactor <= 0.0) {
	...
	float currentVelocity = length(uMouseVelocity);
	float clampedVelocity = min(currentVelocity, uMaxVelocity);
	float velocityStrength = clampedVelocity * uVelocityStrength;
	float centerWeight = 1.0 - (dist / blurSize);
	velocityStrength *= centerWeight * centerWeight;
	vec2 velocityDir = safeNormalize(uMouseVelocity);
	
	float sigma = blurFactor * (uBlurAmount + velocityStrength * 20.0);
...
vec4 plusLighter(vec4 source, vec4 destination) {
	// Increase the strength of the source color
	source.rgb *= uAlpha;
	// Make the destination (original layer) completely transparent
	destination.a = 1.0;
...
void main() {
	...
	for(int i = -20; i <= 20; i++) {
		...
		vec2 velocityOffset = velocityDir * x * velocityStrength;
		vec2 offset = vec2(0.0, x * 0.002) + velocityOffset;
